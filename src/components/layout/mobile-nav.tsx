...
import { ClipboardCheck, LayoutDashboard, Settings2, Users2 } from "lucide-react";
...
export function MobileNav() {
  ...
  return (
    <nav className="fixed bottom-6 left-1/2 z-50 flex -translate-x-1/2 items-center gap-1.5 rounded-full border border-white/20 bg-slate-900/90 p-1.5 shadow-2xl backdrop-blur-xl md:hidden">
      {links.map((link) => {
  ...
  return (
    <header className="sticky top-0 z-40 flex h-16 items-center border-b border-slate-100 bg-white/80 px-4 backdrop-blur-lg md:hidden">
      <div className="flex items-center gap-3">
        ...
        <p className="text-sm font-bold tracking-tight text-slate-900 uppercase">Tuition Suite</p>
      </div>
    </header>
  );
}
